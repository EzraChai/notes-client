...
export default function Todo(){
    ...
    const deleteHandler = async () =>{
        ...
        }).then(router.push('/memo?done=deleted'));
    }
        ...
        <div>
            ...
            <div>
                <div className="">
                    {data?(
                        <>
                            <div className="w-7/8 mx-auto mt-20 bg-white dark:bg-indigo-900 rounded-xl shadow-lg w-80 md:w-full max-w-7xl transition transform hover:-translate-y-1 hover:scale-100">
                                <div className="text-indigo-700 dark:text-gray-50 text-center text-3xl p-3 capitalize font-bold">{data.todo.title}</div>
                            </div>
                            <div className="w-7/8 mx-auto mt-10 bg-white dark:bg-indigo-900 dark:border-black dark:border-4 rounded-xl shadow-lg w-80 md:w-full max-w-7xl transition transform hover:-translate-y-1 hover:scale-100    ">
                                    {/* <p className="text-gray-800 text-center text-2xl p-5 block">{word}</p> */}
                            ...
                            <div>
                            <button onClick={deleteHandler} className = " fixed z-10 bg-red-700 hover:bg-red-500 transition text-white w-16 h-16 md:w-20 md:h-20 border-none cursor-pointer rounded-full bottom-4 right-4 md:bottom-12 md:right-12" >
